//! solution for https://adventofcode.com/2015/day/13 part 2
...
fn includeZorro() !void
{
    for (people.items) | person |    
    {
        const key1 = try makeKey("zorro", person);
        const key2 = try makeKey(person, "zorro");

        try feelings.put(key1, 0);
        try feelings.put(key2, 0);
    }
    
    try people.append("zorro");
}
