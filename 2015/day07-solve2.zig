//! solution for https://adventofcode.com/2015/day/7 part 2
...
const adjustment = 46065; // ** YOU MUST CHANGE (or not) THIS VALUE ACCORDING TO YOUR PUZZLE INPUT **
...
// adjusting to new start /////////////////////////////////////////////////////

fn adjustToNewStart() void
{
    for (0..rules.items.len) |index|
    {
        const rule = &rules.items[index];
        
        if (rule.kind != .assign) { continue; }
        if (! std.mem.eql(u8, rule.destiny, "b")) { continue; }
        
        rule.valueA = adjustment;
    }
}
