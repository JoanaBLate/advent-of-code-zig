//! solution for https://adventofcode.com/2015/day/16 part 2
...
fn processInputLine(line: []const u8) !void
{
    ...
    for (0..3) | turn |
    {
        ...
        if (std.mem.eql(u8, label, "cats"))
        {
            if (value <= profileValue) { return; } else { continue; }
        }
        
        if (std.mem.eql(u8, label, "trees"))
        {
            if (value <= profileValue) { return; } else { continue; }
        }
        
        if (std.mem.eql(u8, label, "goldfish"))
        {
            if (value >= profileValue) { return; } else { continue; }
        }
        
        if (std.mem.eql(u8, label, "pomeranians"))
        {
            if (value >= profileValue) { return; } else { continue; }
        }
