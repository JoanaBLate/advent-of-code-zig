// solution for https://adventofcode.com/2020/day/19 part 1
...
pub fn main() !void
{
 ...
 // while (try replaceWithLiterals()) { } // too slow to replace everything!
 ...
 // #0:  [8][11]
 // #8:  [42]
 // #11: [42]aabbbbba [42]aabbaaba [42]aabaaaba [42]aaaabbba...
    
 // doing more replacements at run time would waste time and memory
 // rule 8 may be replaced in our mind:
 
 // #0:  [42][11]
 // #11: [42]aabbbbba [42]aabbaaba [42]aabaaaba [42]aaaabbba...

 // we realize that the pattern is made of three segments:
 // 1) one segment from rule #42
 // 2) annother segment from rule #42
 // 3) one segment from rule #11 EXCLUDING the link (#42)
 
 
    const segments11 = try listOfLiteralsFrom(allNodes[11].rule);
    const segments42 = try listOfLiteralsFrom(allNodes[42].rule);
    
    const expectedLength: u32 = 3 * 8;
    ...
    {
        if (message.len != expectedLength) { continue; }
        
        if (! segmentMatches(message[0..8],   segments42)) { continue; }
        if (! segmentMatches(message[8..16],  segments42)) { continue; }
        if (! segmentMatches(message[16..24], segments11)) { continue; }
...
fn listOfLiteralsFrom(source: []const u8) !std.ArrayList([]const u8)
{
    var list = std.ArrayList([]const u8).init(allocator);

    var tokens = std.mem.tokenizeAny(u8, source, " [42]");
...
fn segmentMatches(segment: []const u8, list: std.ArrayList([]const u8)) bool
{
